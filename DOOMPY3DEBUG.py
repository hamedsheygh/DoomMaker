...
def shoot():
    ...
    # Debug: Draw a blue line to visualize the ray
    if shoot_ray.hit:
        # Draw line to the point of hit
        hit_point = shoot_ray.world_point
        line = Entity(model=Mesh(vertices=[player.position, hit_point], mode='line', thickness=0.5),
                      color=color.blue)
    else:
        # Draw line for the full ray distance if it doesn't hit anything
        end_point = player.position + camera.forward * 10
        line = Entity(model=Mesh(vertices=[player.position, end_point], mode='line', thickness=0.5),
                      color=color.red)
    
    # Destroy the line after 1 second
    invoke(destroy, line, delay=10)
